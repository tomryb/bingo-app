...
export default function BingoBoard({ size, user, roomId }: BingoBoardProps) {
  ...
  const [selected, setSelected] = useState<boolean[]>(Array(size * size).fill(false));
  ...
  useEffect(() => {
    const onMark = ({ user: fromUser, index }: { user: string; index: number }) => {
      toast(`${fromUser} zaznaczył: "${boardTexts[index]}"`)
    }
  ...
  return (
    <div className="w-full">
      <div className="mb-4 text-sm text-slate-300">Grasz jako: <span className="text-white font-semibold">{user}</span>{roomId ? ` — Pokój: ${roomId}` : ''}</div>
